...
const Order = ({ order, handleOrderState, delieveringStatus = false }) => {
     ...
     <div className="w-[95%] mx-auto border-dark/20 rounded-[10px] p-2 mb-6 shadow-boxShadow1 cursor-pointer">
        ...
        <div className="flex mt-4 gap-2 text-[14px]">
           ...
           <Link to={`/commande/${id}`} className="w-full">
              <button className="border-solid border-[2px] border-dark/10 p-1 rounded-md w-full" onClick={handleOrderState}>
                 Détails
              </button>
           </Link>

           <Link to={activeLink} className="w-full">
              <button className="rounded-md w-full border-solid border-[2px] border-primary bg-primary whitespace-nowrap p-1 text-light font-medium"  onClick={handleOrderState}>
                 {expressiveAction}
              </button>
           </Link>
        </div>
